...
func FilterToModel(filter *inventoryV1.PartsFilter) model.PartsFilter {
	...
	if len(filter.Names) > 0 {
		nameSet = append(nameSet, filter.Names...)
	}
	categorySet := make([]int32, 0, len(filter.Categories))
	if len(filter.Categories) > 0 {
		for _, v := range filter.Categories {
			categorySet = append(categorySet, CategoryToModel(v))
		}
	}
...
func ListPartsToProto(parts []model.Part) []*inventoryV1.Part {
	data := make([]*inventoryV1.Part, 0, len(parts))
	for _, v := range parts {
		data = append(data, InventoryToProto(v))
	}
	return data
}

func InventoryToModel(part *inventoryV1.Part) model.Part {
